...
public sealed record ExperimentLiveMonitoringSnapshot(
    ...
    bool HasWebcamSignal,
    long? WebcamUpdatedAtUnixMs,
...
public sealed record ExperimentSessionSnapshot(
    ...
    WebcamSensingStatusSnapshot WebcamStatus,
...
)
{
    public ExperimentSessionSnapshot Copy()
    {
        return new ExperimentSessionSnapshot(
            ...
            SignalSources?.Copy() ?? SensingSignalSourcesSnapshot.Default.Copy(),
            WebcamStatus?.Copy() ?? WebcamSensingStatusSnapshot.Default.Copy(),
