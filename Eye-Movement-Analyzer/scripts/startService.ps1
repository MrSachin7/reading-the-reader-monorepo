...
if (-not (Test-Path $venvPython)) {
    Write-Host "Creating local virtual environment for Eye-Movement-Analyzer..."
    & $python -m venv (Join-Path $serviceRoot ".venv")
    $python = $venvPython
}

Write-Host "Installing Eye-Movement-Analyzer package into local virtual environment..."
& $python -m pip install -e $serviceRoot | Out-Host

Push-Location $serviceRoot
try {
    Write-Host "Starting mock Eye-Movement-Analyzer service..."
