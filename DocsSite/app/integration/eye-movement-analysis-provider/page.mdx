...
`tokenText` is the exact rendered text from the participant browser token element. `tokenKind` comes from the rendered token metadata and is normally `word` in the current phase because fixation hit-testing targets word tokens. For stale, outside-area, and no-token-hit observations, both values are `null`.
...
type ReadingGazeObservationSnapshot = {
  ...
  isStale: boolean
  staleReason: "none" | "no-point" | "point-stale" | "outside-reading-area" | "no-token-hit"
}
```
