...
- `ws://localhost:5190/ws/module-provider`
- `wss://<host>/ws/module-provider`

All external module types connect to this single endpoint. The `moduleId` field in the envelope routes each message to the correct module handler.

For AI decision providers, use `moduleId = "interventions"`.
...
- It will send only provider-protocol messages on `/ws/module-provider`.
...
sequenceDiagram
    ...
    ExternalAI->>Backend: WebSocket connect /ws/module-provider
    ExternalAI->>Backend: providerHello (moduleId: "interventions")
...
The module provider framework uses `moduleId` to route messages. Each module can evolve its message contracts independently.

If the interventions module contract changes incompatibly, update the capability discovery docs and bump the relevant message type names rather than silently overloading existing types.
