...
- External module providers use `/ws/module-provider`

Do not reuse the browser protocol for the module provider. The external provider is a separate actor with a separate contract.
...
1. Open a WebSocket connection to `/ws/module-provider`.
2. Register itself with `providerHello` and include `moduleId = "interventions"`.
...
- Local dev: `ws://localhost:5190/ws/module-provider`
- HTTPS dev/prod: `wss://<host>/ws/module-provider`
...
- `moduleId`: required module identifier, use `"interventions"` for AI decision providers
...
sequenceDiagram
    ...
    Provider->>WS: Connect /ws/module-provider
