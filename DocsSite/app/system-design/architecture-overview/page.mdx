...
- **REST endpoints** (FastEndpoints under `/api`) for eye-tracker discovery, input mode settings, and explicit start/stop.
...
flowchart LR
    ...
    FE -->|HTTP /api/sensing-mode/settings| API
    ...
    SESS --> MODE[ISensingModeSettingsService]
...
- `eyeTracker`: normal Tobii-backed operation. Eyetracker discovery, licence handling, calibration, and validation remain required before session start.
- `mouse`: demo mode for running the frontend without Tobii hardware. Eyetracker and calibration readiness gates are skipped, `eyeTrackerDevice` remains nullable, and the participant reading page sends pointer coordinates as synthetic gaze samples.

Mode settings are exposed through:

- `GET /api/sensing-mode/settings`
- `PUT /api/sensing-mode/settings`

The settings response shape is:
...
The update request uses the backend request casing:
...
Mode changes are blocked while an experiment session is active. In that case, the update endpoint returns `409 Conflict` and the settings response reports `canChangeMode: false` with a block reason. This prevents mixing Tobii-backed and mouse-backed samples inside one session export.

Mouse samples enter the same gaze pipeline as hardware samples. The frontend sends `mouseGazeSample` frames only in mouse mode and only while the pointer moves. The current participant reading page throttles sending to one sample every `33ms`, or about `30.3` samples per second. Coordinates are normalized against the browser viewport and copied into both eyes with `leftEyeValidity` and `rightEyeValidity` set to `Valid`.
...
- If there is no Tobii device on Windows, start will fail in `eyeTracker` mode.
- In `mouse` mode, session start does not require Tobii hardware, licence upload, calibration, or validation.
- Mouse mode sends synthetic gaze only while pointer movement events occur, so the effective sample rate can be lower than 30 Hz when the mouse is still or the browser throttles the tab.
