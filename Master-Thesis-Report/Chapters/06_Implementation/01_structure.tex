...
\caption[Code structure of the two monorepo roots.]{Code structure of the two monorepo roots. In the backend solution every project reference points inward (\texttt{WebApi}, the composition root, depends on \texttt{Application} and all infrastructure; each infrastructure adapter and \texttt{Application} depend inward towards \texttt{Domain}), which is the dependency rule of \cref{sec:design-style} enforced by the build rather than merely intended. The front-end is a single Next.js application whose source separates thin routing (\texttt{app/}) from feature UI (\texttt{modules/pages}), shared state and transport (\texttt{redux}, \texttt{lib}), and shared components.}
