...
The platform is a monorepo with two independent roots, a .NET backend solution under \texttt{Backend/} and a Next.js front-end under \texttt{Frontend/}, with no shared workspace manifest binding them, so that each is built and versioned on its own.
...
\caption[Curated layout of the monorepo.]{Curated layout of the monorepo (selected folders only). \texttt{core} holds domain entities and application ports; \texttt{infrastructure} holds the adapters; \texttt{WebApi} is the composition root. The front-end separates routing (\texttt{app/}) from feature UI, shared state, and shared components.}
\label{fig:impl-tree}
\end{figure}
...
\caption[Code structure of the two monorepo roots.]{Code structure of the two monorepo roots. Every backend project reference points inward towards \texttt{Domain}, enforcing the dependency rule of \cref{sec:design-style} at the build level. The front-end separates routing (\texttt{app/}) from feature UI, state, and shared components.}
...
\end{minted}
\caption{Composition root in \texttt{WebApi/Program.cs} (abbreviated). Each \texttt{Install\dots Module} method is defined in the module it installs and is the only place that binds a core port to a concrete adapter.}
...
\begin{table}[htbp]
\centering
\caption[The realised technology stack by tier.]{The realised technology stack by tier, with the resolved version and the role each part plays. The selection rationale is argued in \cref{sec:technology-selection}; the versions and supporting libraries recorded here are its concrete realisation.}
