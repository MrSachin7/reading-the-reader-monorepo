...
\caption{The Reading the Reader concept as published by the project. A tracker of eyes, pupils, and posture feeds feature extraction and a classifier, trained offline on a database of readers, which emits an intervention signal that the reading application renders as a typographic adaptation for the reader, closing the loop. Figure reproduced from the project description \cite{dtucompute2025rtr}.}
...
\caption{Our modular interpretation of the adaptive reading loop. The reader views text on a screen while an eye tracker beneath it captures gaze and pupil signals, which enter a pipeline of four independently replaceable modules (sensing, analysis, decision, and intervention); the intervention module returns adapted text to the screen, closing the loop. Each module is one of several interchangeable implementations behind a stable contract (grey). The decision seam additionally admits an external or AI provider (red, defined as a contract and out of scope in this thesis) and the researcher (teal), who monitors the live loop and can intervene manually.}
