...
%% ── Associations ─────────────────────────────────────────────────────────
% Researcher → fifteen use cases spanning all five bands
% (uc-calibrate added: researcher initiates and reviews calibration)
...
The researcher reviews the resulting quality metrics and may repeat the
...
Fig.~\ref{fig:activity-session} traces the control flow of a complete experimental session, synthesising the preparation path of UC-1 and the live path of UC-2 into a single workflow.
The diagram remains at the analysis level: every node denotes an activity and every branch a condition, with no reference to system components or internal mechanisms.
...
\begin{tikzpicture}[
  ...
  dec/.style={
    draw, diamond, aspect=2.2, thick,
    inner sep=1pt, align=center, font=\scriptsize, fill=white
  },
  term/.style={draw, circle, fill=black, minimum size=0.32cm, inner sep=0},
  flow/.style={->, >=latex, thick},
  elbl/.style={font=\scriptsize, fill=white, inner sep=1.5pt}
]

%% ── Nodes ─────────────────────────────────────────────────────────────────────
\node[term] (start)        at (0,   0.0)   {};
\node[act]  (prepare)      at (0,  -1.25)  {Prepare reading materials\\and templates};
\node[act]  (configure)    at (0,  -2.5)   {Configure session settings};
\node[act]  (connect)      at (0,  -3.75)  {Connect and select\\eye tracker};
\node[act]  (validate)     at (0,  -5.0)   {Validate device licence};
\node[act]  (perform)      at (0,  -6.25)  {Perform calibration};
\node[dec]  (d1)           at (0,  -7.75)  {Calibration\\passed?};
\node[act]  (startses)     at (0,  -9.25)  {Start session};
\node[act]  (present)      at (0, -10.5)   {Present reading item};

% Fork bar
\fill (-5.6, -11.45) rectangle (5.6, -11.35);

% Concurrent lanes
\node[act]  (laneL)        at (-4.0, -12.5) {Participant reads\\presented text};
\node[act]  (laneM)        at ( 0.0, -12.5) {Record gaze and derive\\fixation events};
\node[act]  (laneR)        at ( 4.0, -12.5) {Monitor\\live session};

\node[dec]  (d2)           at (0, -14.1)   {Intervention\\warranted?};
\node[act]  (apply)        at (0, -15.7)   {Apply or propose\\intervention};

% Join bar
\fill (-5.6, -17.05) rectangle (5.6, -17.15);

\node[dec]  (d3)           at (0, -18.6)   {Comprehension\\quiz?};
\node[act]  (quizbox)      at (4.9, -18.6) {Administer\\comprehension quiz};
\node[dec]  (d4)           at (0, -20.9)   {More reading\\items?};
\node[act]  (export)       at (0, -22.7)   {Export session data};
\node[term] (end)          at (0, -24.0)   {};
\draw[thick] (end) ++(0,0) circle (0.26);

%% ── Sequential flow ───────────────────────────────────────────────────────────
\draw[flow] (start)     -- (prepare);
\draw[flow] (prepare)   -- (configure);
\draw[flow] (configure) -- (connect);
\draw[flow] (connect)   -- (validate);
\draw[flow] (validate)  -- (perform);
\draw[flow] (perform)   -- (d1);

% Calibration loop (no)
\draw[flow] (d1.east) -- (2.7,-7.75) -- (2.7,-6.25) -- (perform.east)
  node[elbl, above, pos=0.3] {no};
% Calibration passed (yes)
\draw[flow] (d1.south) -- (startses) node[elbl, right, pos=0.5] {yes};

\draw[flow] (startses) -- (present);

% Fork
\draw[flow] (present) -- (0,-11.45);
\draw[flow] (0,-11.35) -- (laneL);
\draw[flow] (0,-11.35) -- (laneM);
\draw[flow] (0,-11.35) -- (laneR);

% Lane M chain
\draw[flow] (laneM) -- (d2);
\draw[flow] (d2.south) -- (apply) node[elbl, right, pos=0.5] {yes};
\draw[flow] (apply) -- (0,-17.05);
% Lane M "no" routes past the activity into the join
\draw[flow] (d2.east) -- (2.0,-14.1) -- (2.0,-17.05) node[elbl, above, pos=0.4] {no};

% Join (lanes L and R into the bar)
\draw[flow] (laneL.south) -- (-4.0,-17.05);
\draw[flow] (laneR.south) -- (4.0,-17.05);

% After join
\draw[flow] (0,-17.15) -- (d3);

% Quiz branch
\draw[flow] (d3.east) -- (quizbox.west) node[elbl, above, pos=0.45] {yes};
\draw[flow] (quizbox.south) -- (4.9,-20.9) -- (d4.east);
\draw[flow] (d3.south) -- (d4) node[elbl, right, pos=0.5] {no};

% Multi-item loop (yes)
\draw[flow] (d4.west) -- (-6.4,-20.9) -- (-6.4,-10.5) -- (present.west)
  node[elbl, above, pos=0.12] {yes};
% No more items
\draw[flow] (d4.south) -- (export) node[elbl, right, pos=0.5] {no};
\draw[flow] (export) -- (end);

\end{tikzpicture}%
}
\caption{%
Activity diagram for a complete experimental session.
...
reading phase.
The diagram stays at the analysis level, describing what happens and under what
conditions rather than how the system is structured internally.%
}
