...
The \textbf{Researcher} is the primary actor and is associated with fourteen of the
...
\node[uc] (uc-license)   at (13.0, 23.7) {Validate Device\\Licence};
...
\node[uc] (uc-decision)  at (13.0, 14.2) {Control Decision\\Mode};
...
\caption[UML use case diagram for the platform.]{%
...
\textit{Validate Device Licence} provisions the device licence before any gaze data
...
reading interface on a second screen and activates the researcher console.
...
analysis signals.
In autonomous mode it applies typographic changes directly; in advisory mode it
emits a proposal that the researcher approves or rejects through \textit{Control
Decision Mode}.
...
identically in the session record and on the researcher console.
...
\caption[Swimlane activity diagram for a complete session.]{%
Swimlane activity diagram for a complete experimental session, partitioned into
Researcher, System, and Participant lanes.
Rounded boxes are activities; diamonds are decisions; horizontal bars bound the
concurrent live-reading phase.
The intervention activity straddles Researcher and System because it may be
applied autonomously or proposed by the system and approved by the researcher.%
}
