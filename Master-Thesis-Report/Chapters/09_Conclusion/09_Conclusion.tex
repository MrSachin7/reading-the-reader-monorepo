...
Two findings we state without softening: the automated decision path was exercised within budget in a single validation run rather than at campaign scale, and the context-preserving restore moves the reader too far on small reflows.
...
With reproducible, replayable records now in hand, the platform turns the controlled efficacy study and the campaign-scale AI-driven decision-making it deliberately left out of scope from rewrites of core code into configuration choices for the work that follows.
