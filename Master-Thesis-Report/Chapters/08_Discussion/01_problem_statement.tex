...
The controlled sweep quantifies how far: under that original strategy the reader's word was displaced by a median of \SI{104}{px}, three times the \SI{34}{px} of no preservation at all, because the restore snapped the committed sentence to the top of the reading area regardless of where the reader sat (\cref{sec:eval-context}).
That the reader nonetheless resumed faster in the live sessions, where preservation ran with exactly this over-repositioning geometry, is therefore hard to attribute to positional stability, since the geometry was actively destabilising; it is best explained by the highlight cue that accompanies the restore, which flashes the restored sentence and draws the eye straight back to the resume point, an account consistent with the prior project finding that highlighting was the component that drove the significant recovery improvement \cite{jensen2025context}.
The stronger reading this invites is that the highlight, not the positional restore, is the more important of the two for recovery: the behavioural gain appeared even while the geometry worked against it.
We hold this as a hypothesis rather than a result, because the live condition bundled the highlight with the restore and cannot separate them, the confound recorded in \cref{sec:eval-threats}; the revised restore now makes the test feasible, by comparing the offset-preserving restore with and without the highlight (\cref{sec:disc-future-work}).
...
Why the over-repositioning happens is covered in \cref{sec:disc-lessons}; acting on it, we revised the restore so that a layout reflow holds the captured reading position at its prior on-screen offset, and a controlled re-run of the deterministic sweep confirms this removes the over-repositioning, though re-evaluating the revised restore with readers remains future work (\cref{sec:eval-context}, \cref{sec:disc-future-work}).
