...
The strongest case is the connector that wraps a collaborator's reading-difficulty model.
It integrates a model we did not write, against a contract we did not design around it, and adding it changed no file in the backend.
This is the closest evidence we have that the contract works for someone other than its authors.
...
One caveat, which we develop in \cref{sec:disc-limitations}, is that we built the reference providers ourselves, so they show that the contract is sufficient but not that it is independently validated; the external model is the only evidence that goes beyond our own work.
...
The honest gap is the automated decision path.
Its latency is instrumented, both inside the backend and out to an external provider, but it was never exercised, because all eight recorded sessions were run in manual mode.
The budget is therefore shown for transport, not yet for an automated decision.
We see this as a limit of the dataset, not of the architecture: the path and its instrumentation exist but were not used, and the run that would close this gap is described in \cref{sec:disc-future-work}.
...
We read these two findings together as support for the design choice, not as a problem with it.
