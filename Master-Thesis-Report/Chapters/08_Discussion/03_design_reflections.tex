...
Instead of assuming that an intervention preserves the reader's place, the reader measures and records a graded residual for every change, and it was this measurement that revealed the over-repositioning of the semantic-restart strategy rather than letting it go unnoticed (\cref{sec:eval-context}).
...
This separation is what the over-repositioning finding refines rather than overturns: committing at a boundary and re-anchoring across it is sound, and only the arithmetic of the restore on small reflows needs improving in future work (\cref{sec:disc-future-work}).
