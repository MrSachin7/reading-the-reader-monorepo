...
- **Gradual reveal within the section:**
  1. Start at a **system-context** altitude: the platform as a single box surrounded by its actors (researcher, participant, Tobii device, external decision provider). This is the only fully-abstract picture and orients the reader who has never seen the system.
  2. Zoom once to a **container/topology** view: the small set of runtime pieces (researcher-facing UI, participant-facing reader, the backend host, the high-frequency channel, the request channel, the local session store, the external-provider seam). This is where the frontend/backend/sockets/store picture is paid off — **once**.
...
- **Diagrams (flag to user, do not auto-generate):** a **C4 System-Context diagram** then a **C4 Container diagram**. The earlier `5.2.svg` was deleted; both are to be re-created fresh.
