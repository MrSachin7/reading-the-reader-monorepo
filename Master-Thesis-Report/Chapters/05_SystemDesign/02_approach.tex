...
\caption[Overview of the Reading the Reader platform.]{Overview of the Reading the Reader platform as a researcher--participant adaptive loop. The participant reads on one screen while a Tobii eye tracker senses gaze; the backend adapts the text and mirrors the gaze and derived analysis to the researcher's console, and an external decision provider may attach through a well-defined API (out of scope, C3).}
