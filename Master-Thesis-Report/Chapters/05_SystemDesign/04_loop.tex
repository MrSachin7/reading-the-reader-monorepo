...
\caption[One iteration of the real-time adaptive loop.]{One iteration of the real-time adaptive loop across the two front-end surfaces: gaze is mirrored over the dedicated real-time channel, the core projects oculomotor events and (in advisory mode) awaits researcher approval before committing the intervention at a controlled boundary, and the adapted text returns to both surfaces. The backend self-calls denote the analysis, decision, intervention, and persistence steps performed within the core, and the checkpoint is written best-effort.}
...
\caption[Lifecycle of a decision proposal.]{Lifecycle of a decision proposal. A proposal from the active decision provider (built-in or external) enters a pending state and resolves into exactly one terminal outcome; in advisory mode the researcher approves or rejects it, while in autonomous mode the platform applies it directly.}
