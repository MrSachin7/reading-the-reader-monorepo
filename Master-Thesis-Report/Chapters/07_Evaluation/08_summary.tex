...
For the sensing pipeline (RQ2) the eye tracker streamed at its rated \SI{90}{\hertz} with validity above 92\%, and the client transport latency stayed within the \SI{100}{\milli\second} budget for every sample, while the automated decision-path latencies, exercised in one advisory-mode validation run, also stayed within budget, on evidence we treat as a single-run check rather than a campaign.
