...
FR21 Session replay playback            & Replay reconstruction present (I) & Implemented \\
...
The calibration that gated each session passed with an average accuracy between \ang{0.30} and \ang{0.62} and an average precision between \ang{0.08} and \ang{0.33}.
...
The recorded sessions were operated entirely through the researcher console: the session could not start until the setup prerequisites and calibration had passed, the interventions in \cref{tab:eval-sessions} were applied from the control panel during the run, and the participant's view was mirrored to the researcher throughout.
...
Session replay, which reconstructs a recorded session in the reading interface, is present in the system and is marked ``Implemented'' in \cref{tab:req-coverage}; a demonstration of replay is left as supporting material.
