...
The platform was developed by a team of two as an iterative build, with each iteration targeting a coherent vertical slice of functionality (from sensing to decision to intervention to researcher interface) rather than building one layer at a time.
...
Within that Develop phase, divergence took the form of exploring design and integration options inside each vertical slice (rather than prototyping competing whole-system architectures in parallel), with the build-test-correct loop serving as the convergence mechanism that settled each slice on a single approach before it was considered complete.
