...
The methodology of this thesis is structured around the Double Diamond model \cite{designcouncil2023diamond}, a human-centred design framework introduced by the Design Council that organises any design project into two sequential diverge-then-converge cycles.
...
The second diamond covers the solution space: a generative phase that explores and iterates on candidate solutions, followed by a delivery phase that converges on a tested, evaluated artifact.
...
Sec.~\ref{sec:tools-and-technologies} justifies the technology choices for each sub-project.
Sec.~\ref{sec:ethical-considerations} addresses ethical considerations arising from the use of eye-tracking data.

\input{Chapters/03_Methodology/01_research_approach}
\input{Chapters/03_Methodology/02_requirements_elicitation}
\input{Chapters/03_Methodology/03_development_process}
\input{Chapters/03_Methodology/04_evaluation_strategy}
\input{Chapters/03_Methodology/05_tools_and_technologies}
\input{Chapters/03_Methodology/06_ethical_considerations}
\input{Chapters/03_Methodology/07_summary}
