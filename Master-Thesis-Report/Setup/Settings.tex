...
% Code-listing background and frame (token colours come from the Pygments theme).
\definecolor{codebg}   {rgb/cmyk}{0.972,0.974,0.980 / 0,0,0,0.03}
\definecolor{codeframe}{rgb/cmyk}{0.851,0.851,0.871 / 0.02,0.02,0,0.13}
...
% Captions: one step smaller than body text (\small is ~1--2pt below the body
% size) and upright. Italics are NOT used for captions: within the thesis,
% italics are reserved for quotations, equations, and terms in body text
% (supervisor feedback). The label ("Figure 5.1") stays bold so the number
% still stands out.
\captionsetup{belowskip=\parskip,aboveskip=4pt plus 1pt minus 1pt,font={small},labelfont={small,bf}}
...
% Code listings: minted (Pygments-backed) gives real C# and TypeScript lexers
% and genuine IDE colour themes. minted 3.x + latexminted; -shell-escape is set
% in .latexmkrc. Use as: \begin{listing}[htbp] \begin{minted}{csharp} ...
% \end{minted} \caption{...} \label{lst:...} \end{listing}.
\usemintedstyle{default}              % Pygments theme; swap for vs, xcode, friendly, ...
\setminted{
    fontsize=\scriptsize,
    breaklines=true,                  % wrap long lines
    breakanywhere=true,
    autogobble,                       % strip common leading indentation
    numbers=left,
    numbersep=8pt,
    frame=single,
    framesep=5pt,
    bgcolor=codebg,                   % faint editor background
    tabsize=2,
    obeytabs=true,
    xleftmargin=16pt,
}

% Number the `listing` float per chapter (e.g. Listing 6.1) and teach cleveref
% its name, matching the Fig./Tab. convention used elsewhere in the thesis.
\SetupFloatingEnvironment{listing}{name=Listing, within=chapter}
\crefname{listing}{Listing}{Listings}
\Crefname{listing}{Listing}{Listings}

% Make \cref render the same capitalised abbreviations used by the hand-written
% \ref calls in the earlier chapters (Fig.~3, Tab.~2, Sec.~4.1), so cross-reference
% wording stays uniform across the whole thesis. Singular and plural both forced to
% the capitalised form so \cref and \Cref agree regardless of sentence position.
