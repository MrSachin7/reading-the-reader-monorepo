...
if (-not (Test-Path $venvPython)) {
    Write-Host "Creating local virtual environment for the Struggle connector..."
    & $python -m venv (Join-Path $serviceRoot ".venv")
    $python = $venvPython
}

Write-Host "Installing Struggle connector package into local virtual environment..."
& $python -m pip install -e $serviceRoot | Out-Host

Push-Location $serviceRoot
try {
    Write-Host "Starting Reading-The-Struggle connector service..."
    & $python -m struggle_connector
}
finally {
    Pop-Location
}
