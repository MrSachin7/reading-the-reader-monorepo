...
NOTEBOOKS["03_context_preservation"] = nb(
    md("# 03 — Context preservation, on vs off (RQ3)\n\n"
       "**Headline (behaviour):** an oculomotor disruption proxy — the regression rate — "
       "computed identically from the gaze stream with and without preservation.\n\n"
    ...
    code(HEADER),
    md("## 1 — Behaviour: regression rate, with vs without (headline)\n"
       "A backward saccade (regression) is a standard oculomotor sign of reading disruption."),
    code("reg = data.saccades.groupby(['participant','condition'])['isRegression'].mean().unstack('condition')\n"
    ...
    code("# Pooled direction of effect (descriptive).\n"
