...
`),o.markdown=i[0]&&typeof i[0].getAttribute(`data-markdown`)==`string`),t.postMessage(JSON.stringify(o),`*`)}function c(e){try{return window.location.origin===e.source.location.origin}catch{return!1}}function l(t){if(c(t))try{let n=JSON.parse(t.data);n&&n.namespace===`reveal-notes`&&n.type===`connected`?(clearInterval(e),u()):n&&n.namespace===`reveal-notes`&&n.type===`call`&&o(n.methodName,n.arguments,n.callId)}catch{}}function u(){n.on(`slidechanged`,s),n.on(`fragmentshown`,s),n.on(`fragmenthidden`,s),n.on(`overviewhidden`,s),n.on(`overviewshown`,s),n.on(`paused`,s),n.on(`resumed`,s),n.on(`previewiframe`,s),n.on(`previewimage`,s),n.on(`previewvideo`,s),n.on(`closeoverlay`,s),s()}return{id:`notes`,init:function(e){n=e,/receiver/i.test(window.location.search)||(window.location.search.match(/(\?|\&)notes/gi)===null?window.addEventListener(`message`,e=>{if(!t&&typeof e.data==`string`){let t;try{t=JSON.parse(e.data)}catch{}t&&t.namespace===`reveal-notes`&&t.type===`heartbeat`&&i(e.source)}}):r(),n.addKeyBinding({keyCode:83,key:`S`,description:`Speaker notes view`},function(){r()}))},open:r}},Kt=[{id:`sense`,label:`Sense`,x:60,y:16},{id:`analyse`,label:`Analyse`,x:104,y:60},{id:`decide`,label:`Decide`,x:60,y:104},{id:`intervene`,label:`Intervene`,x:16,y:60}],qt={sense:`Sense`,analyse:`Analyse`,decide:`Decide`,intervene:`Intervene`};function Jt(){let e=`http://www.w3.org/2000/svg`,t=document.createElementNS(e,`svg`);t.setAttribute(`viewBox`,`0 0 120 120`);let n=document.createElementNS(e,`circle`);n.setAttribute(`class`,`ring`),n.setAttribute(`cx`,`60`),n.setAttribute(`cy`,`60`),n.setAttribute(`r`,`44`),t.appendChild(n);for(let n of Kt){let r=document.createElementNS(e,`circle`);r.setAttribute(`class`,`node`),r.setAttribute(`data-stage`,n.id),r.setAttribute(`cx`,n.x),r.setAttribute(`cy`,n.y),r.setAttribute(`r`,`10`),t.appendChild(r)}return t}function Yt(e){let t=document.getElementById(`loop-spine`);if(!t)return;t.appendChild(Jt());let n=document.createElement(`div`);n.className=`caption`,t.appendChild(n);let r=t.querySelectorAll(`.node`);function i(e){let i=e&&e.getAttribute(`data-loop-stage`);if(!i){t.classList.remove(`visible`);return}let a=i===`all`?Kt.map(e=>e.id):i.split(`,`).map(e=>e.trim());r.forEach(e=>{e.classList.toggle(`active`,a.includes(e.getAttribute(`data-stage`)))}),n.textContent=i===`all`?`The adaptive loop`:a.map(e=>qt[e]||e).join(` → `),t.classList.add(`visible`)}e.on(`slidechanged`,e=>i(e.currentSlide)),i(e.getCurrentSlide())}var Xt=new F({hash:!0,controls:!0,progress:!0,slideNumber:`c/t`,transition:`slide`,transitionSpeed:`default`,width:1280,height:720,margin:.06,plugins:[Gt]});function Zt(e){let t=document.getElementById(`dtu-logo`);t&&(t.style.opacity=e&&e.dataset.logo===`off`?`0`:`1`)}Xt.initialize().then(()=>{Yt(Xt),Xt.on(`slidechanged`,e=>Zt(e.currentSlide)),Zt(Xt.getCurrentSlide())});